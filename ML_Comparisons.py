...
for (x, y) in generate_arrays_from_file_map2vec(u"big_data/train_wiki_uniform.txt", looping=False):
    X.extend(x[0])
...
joblib.dump(clf, u'big_data/bayes.pkl')  # saves the model to file

# ------------------------------------- END OF TRAINING, BEGINNING OF TESTING -----------------------------------

X = []
final_errors = []
clf = joblib.load(u'big_data/bayes.pkl')
