...
def text2mapvec(doc, mapping, outliers, polygon_size, db, exclude):
    ...
    entities = sorted(entities, key=lambda a_b_c_d1: a_b_c_d1[2], reverse=True)
...
def get_coordinates(con, loc_name):
    ...
    if result:
        ...
        return sorted(result, key=lambda a_b_c_d: a_b_c_d[2], reverse=True)
    else:
        return []
...
def buildMapVec(text):
    ...
    """
    ENCODING_MAP = pickle.load(open(u"data/1x1_encode_map.pkl"), "rb")  # the resolution of the map
    OUTLIERS_MAP = pickle.load(open(u"data/1x1_outliers_map.pkl"), "rb")  # dimensions must match the above
